...
updateSelectInput(session,
                  ...
                  selected = listShinyAppList_Manager[1])
...
observeEvent(input$modal_Create_OK_Manager, {
  ...
  tryCatch({
    ...
    serverInfoFileName <- paste0(serverInfoDir, assignedPort, "_", selectedShinyApp, ".csv")
    ...
    message_Manager <<- paste0("リクエストを受け付けました")
...
observeEvent(input$modal_Delete_OK_Manager, {
  ...
  tryCatch({
    ...
    serverInfoFileName <- paste0(serverInfoDir, targetPort, "_", selectedShinyApp, ".csv")
...
output$textOut_message_Manager <- renderText({
  paste0(format(time_Manager, "%Y/%m/%d %H:%M:%S"), " : ", message_Manager)
})
...
output$tableServerInfo_Manager <- renderDataTable({
  dfServerInfo_Manager_filter <- data.frame()
